...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="container w-100 border border-3 border-primary rounded p-4 mt-3">
            <form action="{{route('role.update', ['role' => $Role])}}" method="post" autocomplete="off">
                @method('PUT')
                @csrf
                {{--PARA ENVIAR FORMULARIOS--}}
                <div class="row g-3">
                    <x-form-element id="name" label="Nombre" required="true" focused="true"
                                    value="{{ $Role->name }}"></x-form-element>
                                        ...
                                        <div class="text-center">
                                            <input type="checkbox" name="permiso[]" class="ver"
                                                   ...
                                                   data-idpermisison="1" value="{{ $item->id }}_1"
                                                    {{ in_array($item->id.'_1', (old('permiso', $Permisos) ?? [])) ? 'checked' : '' }}
                                            >
                                        </div>
                                    </td>
                                    <!-- Crear -->
                                        ...
                                        <div class="text-center">
                                            <input type="checkbox" name="permiso[]" class="crear"
                                                   ...
                                                   data-idpermisison="2" value="{{ $item->id }}_2"
                                                    {{ in_array($item->id.'_2', (old('permiso', $Permisos) ?? [])) ? 'checked' : '' }}
                                            >
                                        </div>
                                    </td>
                                    <!-- Editar -->
                                        ...
                                        <div class="text-center">
                                            <input type="checkbox" name="permiso[]" class="editar"
                                                   ...
                                                   data-idpermisison="3" value="{{ $item->id }}_3"
                                                    {{ in_array($item->id.'_3', (old('permiso', $Permisos) ?? [])) ? 'checked' : '' }}
                                            >
                                        </div>
                                    </td>
                                    <!-- Eliminar -->
                                        ...
                                        <div class="text-center">
                                            <input type="checkbox" name="permiso[]" class="eliminar"
                                                   ...
                                                   data-idpermisison="4" value="{{ $item->id }}_4"
                                                    {{ in_array($item->id.'_4', (old('permiso', $Permisos) ?? [])) ? 'checked' : '' }}
                                            >
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <x-form-buttons routeName="role" isEdit="true"></x-form-buttons>
                </div>
